...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("KAMAL");
MODULE_DESCRIPTION("DAY-16: Workqueue Bottom Half");
...
static struct workqueue_struct *sensor_wq_struct;
...
{
    queue_work(sensor_wq_struct,&sensor_work);
...
static ssize_t sensor_read(struct file *file,char __user *buf,size_t count,loff_t *ppos)
{
    ...
    int len;
    unsigned long flags;
    if(wait_event_interruptible(sensor_wq,data_ready)) return -ERESTARTSYS;
...
static struct file_operations sensor_fops = {.owner=THIS_MODULE,.read=sensor_read};
...
static void __exit sensor_exit(void)
{
    ...
    flush_workqueue(sensor_wq_struct);
    destroy_workqueue(sensor_wq_struct);
